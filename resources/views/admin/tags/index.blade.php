...
@section('main')
    <x-page-header title="Теги" :breadcrumbs="[
        ['name' => 'Дашбоард', 'url' => route('dashboard.index')],
        ['name' => 'Теги', 'url' => route('tag.index')],
    ]"/>
                ...
                <div class="row">
                    <div class="col-7">
                        <x-data-table
                            ...
                            :routes="['tag.edit', 'tag.delete']"
                        />
                    </div>
                    <div class="col-5">
                        @if(!isset($tagsEdit))
                            Новый тег
                            <x-form method="POST" action="{{ route('tag.store') }}" :multipart="false">
                                @csrf
                        ...
                        @endif
                        @if(isset($tagsEdit))
                            <x-form method="POST" action="{{ route('tag.update', $tagsEdit->id) }}" :multipart="false">
                                @csrf
                                @method('PATCH')
                                <x-input label="Имя" id="name" type="text" name="name"
                                         placeholder="Введите имя" icon="fas fa-envelope" :showIcon="false"
                                         :required="true" :disabled="false" :value="$tagsEdit->name"/>
                                <x-button type="submit" class="btn btn-primary" text="Сохранить" position="end"/>
                            </x-form>
                        @endif
                    </div>
